...
public class ArcGauge extends View {
    ...
    public ArcGauge(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.ArcGauge, 0, 0);

        // stroke style
        setStrokeWidth(a.getDimension(R.styleable.ArcGauge_gaugeStrokeWidth, 5));
        setStrokeColor(a.getColor(R.styleable.ArcGauge_gaugeStrokeColor, ContextCompat.getColor(context, android.R.color.darker_gray)));
        setStrokeCap(a.getString(R.styleable.ArcGauge_gaugeStrokeCap));

        // angle start and sweep (opposite direction 0, 270, 180, 90)
        setStartAngle(a.getInt(R.styleable.ArcGauge_gaugeStartAngle, 0));
        setSweepAngle(a.getInt(R.styleable.ArcGauge_gaugeSweepAngle, 220));

        // scale (from mStartValue to mEndValue)
        setStartValue(a.getInt(R.styleable.ArcGauge_gaugeStartValue, 0));
        setEndValue(a.getInt(R.styleable.ArcGauge_gaugeEndValue, 1000));

        // pointer size and color
        setPointSize(a.getInt(R.styleable.ArcGauge_gaugePointSize, 0));
        setPointStartColor(a.getColor(R.styleable.ArcGauge_gaugePointStartColor, ContextCompat.getColor(context, android.R.color.white)));
        setPointEndColor(a.getColor(R.styleable.ArcGauge_gaugePointEndColor, ContextCompat.getColor(context, android.R.color.white)));

        // divider options
        int dividerSize = a.getInt(R.styleable.ArcGauge_gaugeDividerSize, 0);
        setDividerColor(a.getColor(R.styleable.ArcGauge_gaugeDividerColor, ContextCompat.getColor(context, android.R.color.white)));
        int dividerStep = a.getInt(R.styleable.ArcGauge_gaugeDividerStep, 0);
        setDividerDrawFirst(a.getBoolean(R.styleable.ArcGauge_gaugeDividerDrawFirst, true));
        setDividerDrawLast(a.getBoolean(R.styleable.ArcGauge_gaugeDividerDrawLast, true));
...
//        float radius = (width > height ? width/2 : height/2);
        ...
        if (mPointSize>0) {//if size of pointer is defined
            ...
            if (mValue==mStartValue) //use non-zero default value for start point (to avoid lack of pointer for start/zero value)
                canvas.drawArc(mRect, mStartAngle, 1, false, mPaint);
            else
